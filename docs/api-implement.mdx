...
<summary>
```
GET https://new.api.greenleaze.com/v1/price-rules
```
</summary>
...
[
  //...
  {
    ...
    "language": "", // (String, Optional) : Langage used by the shop - iso format (ex: fr or en).
    "customer": {}, // (Object, Optional) : Info about the customer. - See below for the customer format.
    "deliveryOptions": [
       // ...
...
```json title="Address format"
{
  ...
  "description": "" // (String, Optional) : Description of the address.
}
```
...
```json title="Delivery option format"
{
...
Note : The delivery options are optional but allows you to display the delivery options on the GreenLeaze payment page, GreenLeaze will only store the final delivery price.
...
then the customer will be redirected on the same Url (GET request with no parameters except the greenleaze-subscription-id)
